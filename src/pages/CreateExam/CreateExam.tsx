...
import { createExamApi, getExamModifyDetail, getExamModifyDetailApi } from "../../redux/reducers/exam";
...
const CreateExam = (props: Props) => {
  ...
  const { examModifyDetail } = useSelector((state: RootState) => state.examSlice)
  ...
  const formik = useFormik({
    initialValues: examModifyDetail,
  ...
  useEffect(() => {
    let setData = async () => {
      let examDetail: ExamDetailFormModel
      if (examId) {
        await dispatch(getExamModifyDetailApi(Number(examId)))
      } else {
        examDetail = {
          title: '',
          categoryId: null,
        ...
        await dispatch(getExamModifyDetail({ examDetail }))
      }
      ...
      <Form labelAlign='left' labelCol={{ span: 4 }} wrapperCol={{ span: 20 }}>
        <Row gutter={24}>
          ...
          <Col xs={24} sm={24} md={24} lg={12} xl={10} xxl={8}>
            ...
            {userInfo?.roles?.find(roleItem => roleItem === "ADMIN") ? <Form.Item label={'Exam type'}>
              <Select
                style={{ width: 100 }}
