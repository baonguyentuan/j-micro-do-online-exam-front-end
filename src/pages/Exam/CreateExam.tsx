...
import { createExamApi, editExamApi, examGetDetailReceived, getFullExamDetail, updateThumbnailExamApi } from "../../redux/reducers/exam";
...
const CreateExam = ({ status }: Props) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      question: Yup.array().min(AppConfigs.exam.MIN_QUESTION_EXAM, t('exam.Each exam have at least {{min}} question and max {{max}} question', { min: AppConfigs.exam.MIN_QUESTION_EXAM, max: AppConfigs.exam.MAX_QUESTION_EXAM })).max(AppConfigs.exam.MAX_QUESTION_EXAM, t('exam.Each exam have at least {{min}} question and max {{max}} question', { min: AppConfigs.exam.MIN_QUESTION_EXAM, max: AppConfigs.exam.MAX_QUESTION_EXAM })),
      file: Yup.mixed().required(t('detail.file is required')),
    })
  })
  const renderThunbnailInput = () => {
    if (status === Constants.formStatus.EDIT) {
      return <Form.Item label={t('detail.thumbnail')}>
        {file.file ? <img width={150} height={150} src={file.fileSrc} alt={formik.values.examType} /> : <img width={150} height={150} src={typeof formik.values.file === 'string' ? formik.values.file : ''} alt={formik.values.examType} />}
        <Input
          className='my-4'
          value={file.filePath}
          multiple={false}
  ...
  return (
    <div className='size__component py-4' style={{ minHeight: '70vh' }}>
      {status === Constants.formStatus.EDIT ? <h1 className='text-center font-bold text-2xl m-4'>{t('exam.Update exam')}</h1> : <h1 className='text-center font-bold text-2xl m-4'>{t('exam.Create exam')}</h1>}
      <Form labelAlign='left' labelCol={{ span: 4 }} wrapperCol={{ span: 20 }}>
        <Row gutter={24}>
          ...
          <Col xs={24} sm={24} md={24} lg={12} xl={10} xxl={8}>
            ...
            {userInfo?.roles?.find(roleItem => roleItem === "ADMIN") === "ADMIN" || status === Constants.formStatus.EDIT ? <Form.Item label={'Type'}>
              <Select
                style={{ width: 100 }}
                disabled={status === Constants.formStatus.CREATE ? false : true}
